...
QVariant CountryModel::data(const QModelIndex &index, int role) const {
    if (index.column() != 0 || index.row() >= Code.count())
        return QVariant();

    switch (role) {
    case Qt::DisplayRole:
        return displayCountry(Code[index.row()]);
    case Qt::EditRole:
    case Qt::UserRole:
        return Code[index.row()];
    default:
        return QVariant();
    }
}

const QMap<QString, QString> CountryModel::CodeMap = {
    {"", ""},
    {"AF", "Afghanistan"},
    {"AX", "Åland Islands"},
    {"AL", "Albania"},
...
const QStringList CountryModel::Code = CountryModel::CodeMap.keys();
